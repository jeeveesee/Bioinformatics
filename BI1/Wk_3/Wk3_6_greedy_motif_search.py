...
# create the best motifs matrix using greedy motif search
# Cogniterra Bioinformatics Course: Course 1: Week 3: 1.5 Greedy Motif Search
# https://cogniterra.org/lesson/30305/step/4?unit=22386
...
###############################################
# STEP 2:  GREEDY MOTIF SEARCH
###############################################


# Function: Create Profile Matrix
# This function creates a profile matrix from a list of motifs
# 1. Start by creating a DF of 0s (ACGT rows, k columns)
# 2. Count nucletoide occurrences in each column
# 3. Convert counts to probabilities by dividing by number of motifs or len(motifs list)
#################
def create_profile_matrix(motifs):
    import pandas as pd

    k = len(motifs[0])
    profile = pd.DataFrame(0, index=['A', 'C', 'G', 'T'], columns=range(k), dtype=float)
    ...
    profile = profile.div(len(motifs))
...
if __name__ == "__main__":
    ...
    answer = greedy_motif_search(dna_list, k, t)
    print(*answer)
